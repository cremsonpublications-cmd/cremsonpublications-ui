      ...
      <div className="flex items-center justify-center gap-6 mb-8 p-6 bg-gray-50 rounded-xl overflow-x-auto">
        ...
        <div className="flex flex-col items-center flex-shrink-0">
          ...
          <div className="mt-3 text-center max-w-[140px]">
            <p className="text-sm font-semibold text-gray-900 line-clamp-2 leading-tight">{currentProduct.name}</p>
            <p className="text-lg font-bold text-green-600 mt-1">₹{calculatePrice(currentProduct)}</p>
          </div>
        </div>
          ...
          return (
            <React.Fragment key={product.id}>
              <div className="flex flex-col items-center flex-shrink-0">
                ...
                >
                  <div className={`w-32 h-40 bg-white rounded-lg shadow-md overflow-hidden border-2 transition-all duration-200 ${
                    isSelected 
                  ...
                  <div className={`absolute -top-2 -right-2 rounded-full p-1 transition-all duration-200 ${
                    isSelected 
                ...
                <div className="mt-3 text-center max-w-[140px]">
                  ...
                  <p className="text-lg font-bold text-blue-600 mt-1">₹{calculatePrice(product)}</p>
                </div>
              </div>
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 mb-6">
        ...
        <div className="space-y-3">
          {selectedProducts.map((product) => (
            <div key={product.id} className="flex items-center justify-between bg-white rounded-lg p-4 shadow-sm">
              <div className="flex items-center gap-4">
                <div className={`w-2 h-2 rounded-full ${product.id === currentProduct.id ? 'bg-green-500' : 'bg-blue-500'}`}></div>
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    ...
                    {product.classes && (
                      <span className="text-sm text-blue-600 font-medium">Class {product.classes}</span>
                    )}
                  </div>
                </div>
              </div>
              <div className="text-right">
                <p className="text-lg font-bold text-gray-900">₹{calculatePrice(product)}</p>
                {product.id !== currentProduct.id && (
                  <button
                    onClick={() => toggleProductSelection(product)}
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 border-t-2 border-gray-100">
        <div className="text-center sm:text-left">
          <p className="text-2xl font-bold text-gray-900">
            Total Price: <span className="text-blue-600">₹{getTotalPrice().toLocaleString()}</span>
          </p>
          <p className="text-sm text-gray-600 mt-1">
            {selectedProducts.length} item{selectedProducts.length > 1 ? 's' : ''} • Save when you buy together
          </p>
        </div>
        <button
